...
def mfcc_cuda(audio, win_length=256, nfft=512, fs=16000, hop_length=128, numcep=13):
    T = transforms.MFCC( sample_rate = fs,
        n_mfcc = numcep,
        melkwargs = {"n_fft": nfft, "hop_length": hop_length, "n_mels": 23, "center": False})
    mfcc = T(audio)
    return mfcc
...
def find_offset_between_buffers(buffer1, buffer2, fs, hop_length=128, win_length=256, nfft=512, max_frames=2000):
    ...
    correl_nframes = min(len(mfcc1)//2 , len(mfcc2)//2, max_frames)
    ...
    c, earliest_frame_offset, latest_frame_offset = cross_correlation_cuda(mfcc1, mfcc2, nframes=correl_nframes)
    ...
    std = np.std(c) /np.sqrt(len(c))
    if std < 1e-10:
        score = float('inf')
    else:
        score = (c[max_k_index] - np.mean(c)) / std  # standard score of peak
    return {
...
def cross_correlation_cuda(mfcc1, mfcc2,nframes):
    n1, mdim1 = mfcc1.shape
    n2, mdim2 = mfcc2.shape
    o_min = nframes - n2
    o_max = n1 - nframes + 1
    mfcc1_torch = torch.from_numpy(mfcc1).cuda()
    mfcc2_torch = torch.from_numpy(mfcc2).cuda()
    c_right = f.conv1d(mfcc2_torch.transpose(0,1)[None], mfcc1_torch[:nframes].transpose(0,1)[:,None], None, stride=1, groups=26)[0].norm(dim=0)
    c_right = c_right[1:].__reversed__()
    c_left = f.conv1d(mfcc1_torch.transpose(0,1)[None], mfcc2_torch[:nframes].transpose(0,1)[:,None], None, stride=1, groups=26)[0].norm(dim=0)
    c = torch.cat([c_left, c_right]).cpu().numpy()
    return c, o_min, o_max
